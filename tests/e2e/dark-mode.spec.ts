...
test.describe('Dark Mode', () => {
  ...
  test('should apply dark mode CSS classes to html element', async ({ page }) => {
    // Find the visible theme toggle button (works on both desktop and mobile)
    const themeToggle = page.locator(
      'button[aria-label*="theme" i]:visible, button[aria-label*="dark" i]:visible, button[aria-label*="light" i]:visible'
    ).first();
  ...
  test('should update theme toggle button appearance', async ({ page }) => {
    ...
    const somethingChanged = initialLabel !== newLabel || initialContent !== newContent;
    expect(somethingChanged).toBeTruthy();
  });

  test('should apply theme-specific styles to page elements', async ({ page }) => {
  ...
  test('should maintain theme when navigating (single page app)', async ({ page }) => {
    // Find the visible theme toggle button (works on both desktop and mobile)
    const themeToggle = page.locator(
      'button[aria-label*="theme" i]:visible, button[aria-label*="dark" i]:visible, button[aria-label*="light" i]:visible'
    ).first();
    ...
    let hasDark = currentClass?.includes('dark') ?? false;
  ...
  test('should work with system preference detection', async ({ page, context }) => {
