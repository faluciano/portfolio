...
function SkillIcon({
  ...
  return (
    <div className="flex flex-shrink-0 flex-col items-center justify-center gap-2 sm:gap-3">
      {src ? (
        <motion.button
          type="button"
...
const SkillsClient = ({ initialData, onSkillClick }: SkillsClientProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Marquee — full-width, pauses on hover/focus */}
      <div
        className="group/marquee mt-12 w-full overflow-hidden sm:mt-14 md:mt-16"
        role="region"
        aria-label="Technology skills showcase"
      >
        <div
          className="animate-marquee-icons group-hover/marquee:pause-marquee group-focus-within/marquee:pause-marquee flex gap-8 py-4 whitespace-nowrap sm:gap-10 sm:py-6 md:gap-12"
          role="list"
        ...
        >
          ...
          {aggregatedLanguages.map((lang) => (
            ...
            >
              <SkillIcon lang={lang} onSkillClick={handleSkillClick} />
            </div>
          ))}
        </div>
      </div>
